...
void drawHen(float cx){
    ...
    glBegin(GL_QUADS);
      glVertex2f(40,cy-10); glVertex2f(WIN_W-40,cy-10);
      glVertex2f(WIN_W-40,cy); glVertex2f(40,cy);
    glEnd();
    ...
    glBegin(GL_TRIANGLES);
      glVertex2f(cx+40,cy+2); glVertex2f(cx+48,cy+6); glVertex2f(cx+40,cy+8);
    glEnd();
    glColor3f(0,0,0);
    glBegin(GL_POINTS); glVertex2f(cx+28,cy+8); glEnd();
}
...
void drawBasketVisual(){
    ...
    glEnd();
    glColor3f(0.5f,0.3f,0.1f); glLineWidth(3.0f);
    glBegin(GL_LINE_STRIP);
      for(int a=160;a>=20;a-=4)
        glVertex2f(cx+(BASKET_W*0.7f)*cos(a*PI/180.0), cy+28+(BASKET_H*0.2f)*sin(a*PI/180.0));
    glEnd(); glLineWidth(1.0f);
}

void drawEggVisual(){
    if(egg_y<0) return;
    if(egg_type==0) glColor3f(1.0f,1.0f,1.0f);
    else if(egg_type==1) glColor3f(0.1f,0.1f,0.9f);
    else if(egg_type==2) glColor3f(1.0f,0.9f,0.2f);
    else glColor3f(0.45f,0.28f,0.15f);
    glBegin(GL_POLYGON);
...
void updateFrame(int unused){
    if(!paused && mode==1){
        for(int i=0;i<2;i++){
            ...
            if(hen_x[i]<60 || hen_x[i]>WIN_W-120) hen_dir[i]*=-1;
        }
        if(egg_y>=0){
            egg_y-=fall_speed;
            egg_x+=wind*1.2f;
            if(egg_x<10) egg_x=10;
            if(egg_x>WIN_W-10) egg_x=WIN_W-10;
            if(egg_y<=120) egg_y=-200;
        } else if(rand()%100<6) spawnEgg();

        wind_timer++;
        if(wind_timer>60){ wind=(rand()%3)-1; wind_timer=0; }
    }
    glutPostRedisplay();
...
void checkCatches(){
    if(egg_y>=0 && egg_y<=140 && egg_x>=basket_x && egg_x<=basket_x+BASKET_W){
        caught++;
        if(egg_type==0) points+=1;
        else if(egg_type==1) points+=5;
        else if(egg_type==2) points+=10;
...
void display(void){
    ...
    drawBackground();
    if(mode!=1){ drawHen(hen_x[0]); drawHen(hen_x[1]); drawBasketVisual(); drawEggVisual(); }

    if(mode==0){
        glColor3f(1,0.9f,0.6f); drawText(WIN_W/2-80,WIN_H/2+60,"CATCH THE EGG");
        glColor3f(1,1,1);
        ...
        char pts[64]; sprintf(pts,"High Score: %d",high_score);
        drawText(WIN_W/2-40,WIN_H/2-40,pts);
    }
    else if (mode == 1) {
    drawHen(hen_x[0]); drawHen(hen_x[1]);
    drawEggVisual();
    drawBasketVisual();
    ...
    if (time_left <= 0) {
        mode = 0;
        char over[80];
        sprintf(over, "Time's up! Score: %d", points);
        glColor3f(1,0.6f,0.6f);
        drawText(WIN_W/2 - 70, WIN_H/2 + 80, over);
    }
}
else if (mode == 2) {
    ...
    char hs[80]; sprintf(hs, "Best Points: %d", high_score);
